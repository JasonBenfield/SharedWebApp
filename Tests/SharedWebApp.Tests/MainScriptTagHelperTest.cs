...
namespace XTI_WebApp.AspTests
{
    public class MainScriptTagHelperTest
    {
        ...
        {
            Environment.SetEnvironmentVariable("DOTNET_ENVIRONMENT", "Production");
            var input = await setup("/Shared/Current/Group/Action");
            var result = await execute(input);
            var src = result.Attributes[0].Value;
        ...
        {
            Environment.SetEnvironmentVariable("DOTNET_ENVIRONMENT", "Production");
            var input = await setup("/Shared/V1/Group/Action");
            var result = await execute(input);
            var src = result.Attributes[0].Value;
        ...
        private async Task<TestInput> setup(string path = "/Shared/Current/Group/Action", string cacheBust = "")
        {
            ...
            var sharedApp = await factory.Apps().App(SharedAppKey.AppKey);
            var currentVersion = await sharedApp.CurrentVersion();
            return new TestInput(sp, currentVersion);
        }
        ...
        {
            public TestInput(IServiceProvider sp, AppVersion currentVersion)
            {
